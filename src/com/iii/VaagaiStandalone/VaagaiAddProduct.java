...
public class VaagaiAddProduct {
	public static void main(String[] args) {
		...
		for (int i = 0; i < numCategories; i++) {
			...
			CategoryDto catDato = new CategoryDto(categoryID, categoryName);

			ICategory categoryService = new CategoryService();

			boolean categoryadded = categoryService.create(catDato);
			if (categoryadded) {
		...
		for (int i = 0; i < numProducts; i++) {
			...
			boolean productadded = AddUtility.addProduct(productID, catID, productName, productImage, productPackSize,
					productAttribute, actualPrice, discountPrice, rating);
			if (productadded) {
